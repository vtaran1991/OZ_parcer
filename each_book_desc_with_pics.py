...
finalBookList = []
counter = 0
# finalBookList.append(bookInfoScrapper('https://oz.by/books/more10685389.html'))

with open('ALLINONE.csv', 'r') as csvFileToRead:
    reader = csv.reader(csvFileToRead, delimiter=',', quotechar='"')
    for row in reader:
        if re.findall('http', row[5]) != []:
            finalBookList.append(list(bookInfoScrapper(row[5])))
            counter += 1
        else:
            continue

# Сохраняем список книг в файл csv
with open('Final_description_list_with_pic_links.csv', 'w') as csvFile:
    ...
    csvEditor.writerows(finalBookList)
