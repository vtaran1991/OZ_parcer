...
def infoScrapper(pageUrl):
    ...
    for z in range(len(booksPerPage)):
        ...
        newPrice = booksPerPage[z].find('span', {'class': 'item-type-card__btn'}).get_text().strip().replace('\xa0', ' ')
...
responce = requests.get('https://oz.by/books/?availability=1')
responce.raise_for_status()
soup = bs4.BeautifulSoup(responce.text, 'html.parser')
...
for pageNum in range(1, int(pagination)):
    pageUrl = ('https://oz.by/books/?availability=1&page=' + str(pageNum))
...
with open('AVALIABLE.csv', 'w') as csvFile:
